...
import { classNames, isABI, isCode, remove0x } from '@/utils/common'
import { logEvent } from '@/utils/googleAnalytics'
import { addContract, addProject } from '@/store/features/projects/projectsSlice'
import Modal from '@/components/@core/modal'
import Label from '@/components/@core/forms/label'
import ValidationExlamation from '@/components/@core/forms/validationExlamation'
import ValidationError from '@/components/@core/forms/validationError'
...
export default function ModalAddContract({ open, onClose }: Props) {
	...
	const onSubmit = (data: any) => {
		try {
			if (addNewProject) {
				...
				}), router.push('/app/' + projectId + '/' + contractId))
			} else {
				...
				}), router.push('/app/' + selectedProject.id + '/' + id))
			}
			closeModal()
		} catch (error) {
